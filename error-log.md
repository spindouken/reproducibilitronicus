...
### 13. Pipeline Deploy: `gsutil` Not Found Inside Docker Container
**Error:** `/__w/_temp/...sh: 1: gsutil: not found` during the "Upload parquet to GCS" step.
**Why it happened:** The pipeline job runs inside the custom Docker image. That image contains R, restored R packages, Quarto, and system libraries, but it does not contain the Google Cloud SDK. `gsutil` is part of the Cloud SDK, so shell upload commands that work on a normal runner fail inside the container.
**How we fixed it:** Replaced the `gsutil -m cp -r ...` shell steps with `google-github-actions/upload-cloud-storage@v3`, which uploads files to GCS using the credentials from `google-github-actions/auth`. This keeps the R Docker image focused on running the R pipeline instead of turning it into a full Google Cloud CLI image.
**How to recognize it next time:** Exit code `127` usually means "command not found." In a containerized GitHub Actions job, that means the missing command must either be installed in the container or replaced with an action/API call that does not require the CLI.
